# saas-workspace-drizzle-postgres

SaaS starter on the REST + Drizzle workspace kit: Express 5 API + Next.js App Router, pnpm + Turborepo. Email/password and Google OAuth stay as in the auth kit. This folder adds organizations, memberships, Stripe billing, and an org-scoped todo sample.
...
JSON REST (not tRPC). Use **pnpm** only. Never npm or yarn.
...
Unit + Testcontainers Postgres integration + Supertest REST API tests (auth, orgs, tenancy, Stripe webhook signature + idempotency). Google is mocked at `OAuth2Client`. Checkout is mocked at the Stripe client. Docker must be running so Testcontainers can start Postgres; you do not start Postgres yourself. If Docker is not running, the Testcontainers tests are skipped.
