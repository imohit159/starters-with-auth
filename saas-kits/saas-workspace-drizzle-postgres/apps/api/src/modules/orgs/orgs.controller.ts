...
import {
  ...
  type OrganizationIdInputSchema,
...
export async function get(req: Request, res: Response) {
  ...
  const organization = await getOrganization(req.user!.id, input.organizationId);
  res.json({ organization });
}

export async function members(req: Request, res: Response) {
  const input = req.body as OrganizationIdInputSchema;
  const result = await listMembers(req.user!.id, input.organizationId);
  res.json(result);
}
