...
async function findOrgForStripeEvent(event: Stripe.Event): Promise<OrganizationDocument | null> {
...
export async function handleStripeWebhook(payload: Buffer | string, signature: string | undefined) {
  ...
  switch (event.type) {
    case "checkout.session.completed":
      await applyCheckoutSession(orgId, event.data.object as Stripe.Checkout.Session);
      break;
    case "customer.subscription.created":
    case "customer.subscription.updated":
    case "customer.subscription.deleted":
      await applySubscription(orgId, event.data.object as Stripe.Subscription);
      break;
    case "invoice.paid": {
