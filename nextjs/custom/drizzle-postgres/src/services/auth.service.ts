...
import type { AuthResponse, RegisterResponse, Session, User } from "@/types/auth";
import type { ChangePasswordValues, LoginValues, RegisterValues } from "@/schemas/auth";

export async function register(values: RegisterValues) {
  const { data } = await api.post<RegisterResponse>(API_PATHS.register, values);
  return data;
}
...
export async function changePassword(values: ChangePasswordValues) {
  const { data } = await api.post<AuthResponse>(API_PATHS.changePassword, {
...
export async function getSessions() {
  const { data } = await api.get<{ sessions: Session[] }>(API_PATHS.sessions);
  return data.sessions;
}

export async function revokeOtherSessions() {
  const { data } = await api.delete<{ revoked: number }>(API_PATHS.sessions);
  return data.revoked;
}
