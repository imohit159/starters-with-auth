# saas-trpc-workspace-mongo

SaaS starter on the tRPC + Mongo workspace kit: Express 5 API + Next.js App Router, pnpm + Turborepo. Email/password and Google OAuth stay as in the auth kit. This folder adds organizations, memberships, Stripe billing, and an org-scoped todo sample.
...
Collections: `users`, `authidentities`, `sessions`, `authchallenges`, `organizations`, `memberships`, `organizationinvites`, `stripeevents`, `todos`. IDs are Mongo ObjectIds.

Mongoose creates indexes on connect. There is no migration step.
...
Unit + Mongo memory-server integration + Supertest tRPC HTTP tests (auth, orgs, tenancy, Stripe webhook signature + idempotency). Google is mocked at `OAuth2Client`. Checkout is mocked at the Stripe client.
