...
import { generateStripeTestWebhookHeader } from "../../src/utils/stripe";
import { Organization, OrganizationInvite, PLATFORM_ROLE, User } from "../../src/database";
import { createApp } from "../../src/app";
import { resetTestDb, startTestDb, stopTestDb } from "../helpers/db";

vi.mock("../../src/utils/stripe", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../src/utils/stripe")>();
  return {
    ...actual,
...
describe("org, billing, and todo API", () => {
  const app = createApp();
  ...
  it("creates an org, scopes todos by membership, and gates create on subscription", async () => {
    ...
    expect(todo.body.title).toBe("Ship it");
  ...
  it("invites a member who can accept with the matching email", async () => {
    ...
    const row = await OrganizationInvite.findOne({ organizationId: orgId });
    ...
    expect(accepted.body.id).toBe(orgId);
  ...
  it("lets a platform admin grant complimentary access", async () => {
    ...
    const granted = await owner.post(`/api/v1/admin/organizations/${orgId}/access`).send({ days: 30 });
    expect(granted.status).toBe(200);
    expect(granted.body.isSubscribed).toBe(true);
